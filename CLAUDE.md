...
- `useDeckFronts.ts` — imperatively lazy-loads picked cards' front textures (NOT Suspense, so streaming a face never re-mounts/kills animations). Card backs are a procedural canvas texture (`textures.ts`); there is no card-back image asset.
...
- Reduced motion (`src/lib/useReducedMotion.ts`) shortens the shuffle; WebGL/render failures are caught by an `ErrorBoundary` around `<Scene>` in `ReadingRoute`.
...
### Prompt assembly
`src/prompt/assemble.ts` builds a structured context block (question + per-position label/meaning + each drawn card's name/orientation + the orientation-appropriate meanings from the dataset) and selects a system prompt per template (`templates.ts`). All prompts force Simplified-Chinese output. Deep-dive follow-ups reuse the same context plus prior `turns` as conversation history and forbid re-drawing. `useInterpret.ts` orchestrates streaming into the store and auto-saves the `Reading` to IndexedDB after each turn.
...
- Only `features/deck3d/**` imports `three`/`@react-three/*` (keeps the heavy 3D bundle in the lazy reading-route chunk). Use `@/deck/images.ts#cardImageUrl` (three-free) from non-3D code, not `deck3d/textures.ts`.
...
Pure-frontend BYOK means any key the browser uses is visible to the user/page — this is acceptable only because each user supplies their own key. `public/_headers` ships a production CSP whose `connect-src` is limited to the chosen LLM providers (custom providers require relaxing it).
